...
void tcp_synack(uint8_t *buf) {
    ...
    ENC28J60_SendPacket(buf, IP_HEADER_LEN+TCP_LEN_PLAIN+4+ETH_HEADER_LEN);
}
...
void tcp_ack(uint8_t *buf) {
    ...
    ENC28J60_SendPacket(buf,IP_HEADER_LEN+TCP_LEN_PLAIN+ETH_HEADER_LEN);
}
...
void tcp_ack_with_data(uint8_t *buf,uint16_t dlen) {
    ...
    buf[TCP_CHECKSUM] = j>>8;
    buf[TCP_CHECKSUM+1] = j& 0xFF;
    ENC28J60_SendPacket(buf,IP_HEADER_LEN+TCP_LEN_PLAIN+dlen+ETH_HEADER_LEN);
}
