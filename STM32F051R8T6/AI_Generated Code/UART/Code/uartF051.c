...
void UART_SetBaudRate(USART_TypeDef *USARTx, uint32_t BaudRate, uint32_t APBClock) {
    ...
    if (over8) {
        /* Oversampling by 8 */
        integer = ((25 * APBClock) / (4 * BaudRate));
    } else {
        /* Oversampling by 16 */
        integer = ((25 * APBClock) / (2 * BaudRate));
    }
    
    fractional = ((integer % 100) * 16) / 100;
    
    /* Remove integer part */
    integer = integer / 100;
    
    if (over8) {
        fractional = (fractional >> 1);  /* Divide by 2 for OVER8 */
    }
    
    /* BRR = integer | fractional */
    brr = (integer << 4) | fractional;
