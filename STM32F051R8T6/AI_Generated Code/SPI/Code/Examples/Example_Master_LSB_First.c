/**
 * @file Example_Master_LSB_First.c
 * @brief SPI Master Mode - LSB First Transfer Example
 * @description This example demonstrates SPI communication with LSB-first
 *              frame format instead of the default MSB-first.
 * 
 * Hardware Setup:
...
void SPI1_Init_Master_LSB(void);
...
int main(void)
{
    SystemClock_Config();
    SPI1_GPIO_Init();
    SPI1_Init_Master_LSB();
    
    printf("SPI Master - LSB First Transfer Example\r\n");
    printf("========================================\r\n\r\n");
    
    printf("TX Buffer (MSB first in memory): ");
    for (int i = 0; i < 4; i++) printf("%02X ", tx_buffer[i]);
    printf("\r\n");
    
    // Perform transfer
    SPI_SetNSSPin(&hspi1, false);
    SPI_TransferBuffer(&hspi1, (uint16_t*)tx_buffer, (uint16_t*)rx_buffer, 4);
    SPI_SetNSSPin(&hspi1, true);
    
    printf("RX Buffer: ");
    for (int i = 0; i < 4; i++) printf("%02X ", rx_buffer[i]);
    printf("\r\n");
...
//============================================================================
// SPI Initialization - Master with LSB First
//============================================================================
...
void SPI1_Init_Master_LSB(void)
{
    ...
    hspi1.Init.frame_format = SPI_FRAME_FORMAT_LSB_FIRST;  // LSB first!
