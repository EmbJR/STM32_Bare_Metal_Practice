...
static inline void NVIC_SetPriority(int irq, uint8_t priority) {
    if (irq < 0) {
        return;
    }
...
static inline TIM_TypeDef *GetTimer(uint32_t timerId) {
...
static inline void EnableTimerClock(TimerId timer) {
    if (timer <= TIMER_INVALID || timer >= TIMER_COUNT) {
        return;
    }
...
static inline void DisableTimerClock(TimerId timer) {
    if (timer <= TIMER_INVALID || timer >= TIMER_COUNT) {
        return;
    }
    if (TimerOnAPB2[timer]) {
